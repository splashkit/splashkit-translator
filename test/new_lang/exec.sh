...
../../translate --generate clib,pascal,cpp,python,csharp,rust,dart -i "test${LANG_TEST}/new_lang.h" --output ../out -l
...
function run_dart
{
  cp "test${LANG_TEST}/test_program.dart" .
  echo "Running dart program"
  dart ./test${LANG_TEST}/test_program.dart
  dart test_program.dart
}

echo "Do you wish to run the Dart version?"
select yn in "Yes" "No"; do
    case $yn in
        Yes ) run_dart; break;;
        No ) break;;
